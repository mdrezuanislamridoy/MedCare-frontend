...
export default function DashboardPage() {
    ...
    async function loadData() {
      try {
        const [statsData, apptData, docData, queueData]: any = await Promise.all([
          ...
          clinicManagerApi.getQueue().catch(() => []),
        ]);
        ...
        console.warn("Error loading clinic dashboard data:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const todayAppts = stats?.activeAppointmentsToday ?? appointments.length;
  const roomOccupancy = stats?.occupancyRate ?? 0;
  const availableDocs = stats?.totalDoctors ?? doctors.length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Completed" value={appointments.filter(a => a.status?.toLowerCase() === "completed").length} sub="Today so far" icon={Icons.check} color="#059669" />
        <StatCard label="Cancelled" value={appointments.filter(a => a.status?.toLowerCase() === "cancelled").length} sub="Tracking live" icon={Icons.x} color="#DC2626" />
        ...
        <StatCard label="Pending Visits" value={appointments.filter(a => a.status?.toLowerCase() === "pending" || a.status?.toLowerCase() === "scheduled").length} sub="In queue or upcoming" icon={Icons.clock} color="#64748B" />
      </div>
            ...
            <div className="space-y-2">
              {appointments.slice(0, 8).map((t, i) => (
                <div key={t.id || i} className="flex items-center gap-3">
                  <span className="text-xs font-mono text-slate-400 w-14 flex-shrink-0">{t.time || t.slot || "—"}</span>
                  <div className="flex-1 flex items-center gap-2 py-2 px-3 rounded-lg bg-slate-50 border border-slate-100">
                    ...
                    <span className="text-sm text-slate-700 flex-1">{t.patient || t.patientName || "Patient"}</span>
                    <span className="text-xs text-slate-400">{t.doctor || t.doctorName ? `Dr. ${t.doctor || t.doctorName}` : "Doctor"}</span>
                  ...
                  return (
                    <div key={d.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400">{d.specialty || "General Medicine"}</p>
                      </div>
                  ...
                  return (
                    <div key={q.id || idx} className="flex items-center gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-slate-400">{q.waitMins ? `${q.waitMins}m wait` : "Checked in"}</p>
                      </div>
