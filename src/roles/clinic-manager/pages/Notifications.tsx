  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          unreadCount > 0 ? (
            <button onClick={() => setNotifs(prev => prev.map(n => ({ ...n, read: true })))} className="text-sm text-blue-600 hover:underline">
              Mark all read
            </button>
          ) : undefined
        }
      ...
      <Card>
        {notifs.length === 0 ? (
          <div className="py-12 text-center text-slate-400">
            ...
            <p className="text-xs text-slate-400 mt-0.5">Real-time alerts for appointments, staff changes, and patient check-ins will arrive here.</p>
          </div>
        ) : (
            ...
            {notifs.map(n => (
              <div key={n.id} onClick={() => setNotifs(prev => prev.map(item => item.id === n.id ? { ...item, read: true } : item))}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-1">{n.time || "Recent"}</p>
                </div>
              </div>
            ))}
          </div>
