      ...
      <div className="mx-auto max-w-[1536px] px-6 lg:px-12">
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
          {benefits.map(({ icon: Icon, title, description }) => (
            <article
              key={title}
              className="rounded-3xl border border-teal-100/90 bg-white p-7 shadow-card hover:shadow-lift transition-all hover:-translate-y-0.5"
            >
              <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-teal-50 text-teal-600 shadow-xs">
                <Icon className="h-7 w-7" aria-hidden="true" />
              </span>
              <h3 className="mt-5 font-display text-lg sm:text-xl font-bold text-ink">{title}</h3>
              <p className="mt-2.5 text-sm sm:text-base leading-relaxed text-ink-soft">{description}</p>
            </article>
          ))}
        </div>

        <div className="mt-8 grid gap-px overflow-hidden rounded-3xl border border-teal-100/80 bg-teal-100/70 sm:grid-cols-2 lg:grid-cols-4 shadow-sm">
          {dynamicStats.map((stat) => (
            <div key={stat.label} className="bg-white px-8 py-10 text-center">
              <p className="font-display text-4xl font-black tracking-tight text-teal-700 sm:text-5xl">
                {stat.value}
              </p>
              <p className="mt-2 text-base sm:text-lg font-semibold text-ink-soft">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
